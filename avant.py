...
def icon():
    ...
    print(BD + '              Version: ' + C + str(VERSION) + '\n' + W)
...
def show_connections():
    ...
    for connection in psutil.net_connections():
        try:
            ...
            content = '  ' + lhost.strip('::ffff:') + ':' + lport + '  >  ' + rhost.strip('::ffff:') + ':' + rport
            print_length = len(content)
            dashes = (t_length - print_length) * '-'
            print(LG + '  ' + lhost.strip('::ffff:') + W + ':' + Y + lport + W + '  ' + dashes + '>  ' + OR + rhost.strip('::ffff:') + W + ':' + Y + rport + W)
        except ValueError:
            pass
    print()
...
def cmd():
    ...
    while True:
        ...
        if cmd == '':
            ...
            print(Y + BD + 'SHOW PORTS ' + R + '/' + Y + ' PORTS ' + R + '/' + Y + ' P: ' + W + 'Show all listening ports')
            print(Y + BD + 'SHOW CONNECTINOS ' + R + '/' + Y + ' CONS ' + R + '/' + Y + ' C: ' + W + 'Show all network connections')
            print(Y + BD + 'clear: ' + W + 'Clean screen')
            print()
        elif cmd == 'SHOW PORTS' or cmd == 'PORTS' or cmd == 'P':
            opened_ports()
        elif cmd == 'SHOW CONNECTIONS' or cmd == 'CONS' or cmd == 'C':
            show_connections()
        elif cmd == 'CLEAR':
            os.system('clear')
        elif cmd[0] == 'L':
